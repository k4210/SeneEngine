...
public:
    ...
    CommandSignature( UINT NumParams = 0 ) : m_Finalized(FALSE), m_NumParameters(NumParams), m_ByteStride(0)
    {
        Reset(NumParams);
    }
    ...
    ID3D12CommandSignature* Finalize(ID3D12Device* device, ID3D12RootSignature* RootSignature = nullptr );

    ID3D12CommandSignature* GetSignature() const { assert(m_Finalized); return m_Signature.Get(); }

    UINT GetByteStride() const { assert(m_Finalized); return m_ByteStride; }
protected:
